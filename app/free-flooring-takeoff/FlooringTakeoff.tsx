...
import { TakeoffOutputView } from '../free-roof-takeoff/TakeoffOutputView';
...
import { trackFreeToolEvent } from '@/app/lib/trackFreeToolEvent';
import type { QuoteRow } from '@/app/lib/types';

/**
 * Free Flooring Takeoff tool.
 *
 ...
 * differences are the trade config below (flooring components, no pitch) and
 * floor-facing copy. Terminology: floors / floor areas - never roof.
 */
...
const CONFIG = FLOORING_TAKEOFF_CONFIG;
...
export function FlooringTakeoff() {
  ...
  if (stage.phase === 'output') {
    const extras = { planDataUrl: stage.planDataUrl, elapsedMs: Date.now() - stage.startedAt };
    return (
      <TakeoffOutputView
        ...
        {step === 1 && (
          <div className="mt-4 space-y-3">
            ...
            <p className="text-xs text-slate-400">
              Floor areas are measured as drawn - straight off your floor plan. No pitch to worry about.
            </p>
            <button
              onClick={() => setStep(2)}
        ...
        {step === 2 && (
          <div className="mt-4 space-y-4">
            ...
            >
              <input
                type="radio"
                name="component-choice-flooring"
                checked={componentChoice === 'ours'}
                onChange={() => setComponentChoice('ours')}
                className="mt-0.5 w-4 h-4 accent-orange-500"
              />
              <span>
                <span className="block text-sm font-semibold text-slate-900">Use our flooring components</span>
                <span className="block text-xs text-slate-500 mt-0.5">
                  Timber Plank, Carpet, Tile, Underlay, Skirting, Scotia, Transition Strip, Adhesive / Sundries - standard placeholders
                </span>
              </span>
            </label>
            ...
            >
              ...
              <span>
                ...
                <span className="block text-xs text-slate-500 mt-0.5">
                  Same component builder as the app - name, measurement type (floor area, lineal or single item), rates, pricing and waste. Works for any floor covering. Up to {CONFIG.maxCustomComponents}.
                </span>
              </span>
            </label>
                      ...
                      <div key={s.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3 hover:border-orange-200 hover:bg-orange-50/40">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            {s.measurementType === 'lineal' ? 'Lineal' : s.measurementType === 'area' ? 'Floor Area' : 'Single Item'}
        ...
        {step === 3 && (
          <div className="mt-4">
            ...
            >
              ...
              <span className="mt-1 text-xs text-slate-400">Floor plan - PNG, JPG, WebP up to 10 MB - PDF up to 50 MB (pick a page)</span>
              <input
                type="file"

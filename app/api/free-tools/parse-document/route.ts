...
import { resolveFreeToolsTier } from '@/lib/free-tools/resolveTier';
import { parseRateLimitKey, RATE_LIMIT_WINDOW_MS } from '@/lib/free-tools/tiers';
...
// Untyped to avoid dependency on database.types.ts which doesn't exist in NZ repo
let usageClient: ReturnType<typeof createServiceClient> | null = null;
function getUsageClient() {
  if (usageClient) return usageClient;
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) return null;
  usageClient = createServiceClient(url, key, {
...
function logUsage(params: {
  ...
  if (!client) return;
  // Use any cast since NZ repo doesn't have Database types for this table
  (client.from('free_tool_usage') as any).insert({
    ...
    console.warn('[parse-document] usage log failed:', msg);
  });
}

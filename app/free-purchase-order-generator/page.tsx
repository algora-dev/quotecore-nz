...
import { PostGenerationModal } from '../free-tools/shared/PostGenerationModal';
import { buildConvertUrl, parseConvertLines } from '../free-tools/shared/convertLines';

import { ImageUpload, type ParsedUploadResult } from '../free-tools/shared/ImageUpload';
...
function POGeneratorForm() {
  ...
  const [lines, setLines] = useState<POLine[]>(() => {
    ...
    return [{ id: '1', description: '', qty: 0, unit: 'pcs', rate: 0, lineHidden: false }];
  });

  // Country-to-currency mapping for IP-based default
  const COUNTRY_CURRENCY: Record<string, string> = {
    NZ: 'NZD', AU: 'AUD', GB: 'GBP', US: 'USD', CA: 'CAD',
  ...
  useEffect(() => {
    ...
    fetch('/api/geo').then(r => r.json()).then(({ country }) => {
      const code = COUNTRY_CURRENCY[country] || 'NZD';
  ...
  const SESSION_KEY = 'qcp:free-po-session';
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (data.supplierEmail !== undefined) setSupplierEmail(data.supplierEmail);
        if (data.supplierAddress !== undefined) setSupplierAddress(data.supplierAddress);
        if (data.poDate) setPODate(data.poDate);
        if (data.poNumber) setPONumber(data.poNumber);
        if (data.deliveryDate !== undefined) setDeliveryDate(data.deliveryDate);
    ...
    const timer = setTimeout(() => {
      try {
        sessionStorage.setItem(SESSION_KEY, JSON.stringify({
          ...
          companyName, fromName, fromPhone, fromEmail,
          supplierName, supplierEmail, supplierAddress,
          poDate, poNumber, deliveryDate, deliveryAddress,
          notes, footer, footerItalic,
          taxEnabled, taxRate, taxName, hideAllPrices, hideTotals, lines,
        }));
      } catch {}
    }, 500);
    return () => clearTimeout(timer);
  }, [measurementSystem, measurementType, currency, logo, companyName, fromName, fromPhone, fromEmail, supplierName, supplierEmail, supplierAddress, poDate, poNumber, deliveryDate, deliveryAddress, notes, footer, footerItalic, taxEnabled, taxRate, taxName, hideAllPrices, hideTotals, lines]);
  ...
  function resetForm() {
    ...
    setTaxEnabled(true); setTaxRate(15); setTaxName('GST');
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <section className="mb-8 print:hidden">
          <h2 className="text-2xl font-semibold text-slate-900 sm:text-3xl">Free Purchase Order Generator</h2>
          ...
          <div>
            ...
            <div className="mt-4 space-y-2">
              ...
              <details className="rounded-xl border border-slate-200 bg-white">
                ...
                <div className="px-4 pb-4"><p className="text-sm text-slate-600">Yes. The currency selector in the document settings bar supports NZD, AUD, USD, GBP, EUR, and CAD. All amounts in the form and the generated purchase order will use the selected currency symbol.</p></div>
              </details>
              <details className="rounded-xl border border-slate-200 bg-white">
                ...
                <div className="px-4 pb-4"><p className="text-sm text-slate-600">Sign up at the top of the page to gain more free purchase orders and remove the QuoteCore+ branding, or sign up to the full QuoteCore+ app for higher limits and loads of extra features.</p></div>
              </details>
            </div>
          </div>
        </section>

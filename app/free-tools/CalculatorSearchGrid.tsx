        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-5">
          <div className="flex-1">
            <div className="relative">
              ...
              {isSearching && (
                <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 p-1.5 -m-1.5" aria-label="Clear search">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              )}
            </div>
          </div>
          <select value={view} onChange={(e) => setView(e.target.value as 'core' | 'all')} aria-label="Filter calculators by type" className="rounded-lg md:rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 focus:border-orange-500 focus:outline-none">
        ...
        {visibleCalcs.length > 0 ? (
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {visibleCalcs.map((calc) => (
              <Link key={calc.slug} href={calc.ported ? `/${calc.slug}` : `https://quote-core.com/${calc.slug}`} prefetch={false} target={calc.ported ? undefined : '_blank'} rel={calc.ported ? undefined : 'noopener noreferrer'} className={`block bg-white border rounded-xl hover:border-[#FF6B35] hover:shadow-[0_0_8px_rgba(255,107,53,0.08)] hover:bg-orange-50/40 transition-all group ${calc.isCore && view === 'core' && !isSearching ? 'p-5 border-2' : 'p-4 border'}`}>
                <div className="flex items-start gap-3">
                  <div className="p-1.5 rounded-full bg-orange-50 group-hover:bg-orange-100 transition-colors flex-shrink-0">
                    <svg className="w-4 h-4 text-[#BD4A1A]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                  </div>

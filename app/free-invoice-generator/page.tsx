...
import { parseConvertLines } from '../free-tools/shared/convertLines';
                  ...
                  {lines.map((line) => (
                    <div key={line.id} className={`grid grid-cols-12 gap-2 items-start ${line.lineHidden ? 'opacity-40' : ''}`}>
                      ...
                      <div className="col-span-1 flex justify-end gap-1">
                        <button onClick={() => updateLine(line.id, 'lineHidden', !line.lineHidden)} className="p-2 text-slate-400 hover:text-[#BD4A1A] transition" title={line.lineHidden ? 'Show line in invoice' : 'Hide line in invoice (price still counts in total)'}>
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            {line.lineHidden ? (
        ...
        <section className="mt-16 space-y-8 print:hidden">
          <div>
            ...
            <div className="mt-4 space-y-2">
              ...
              <details className="rounded-xl border border-slate-200 bg-white">
                <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-slate-900 hover:text-[#BD4A1A] transition select-none">Does the invoice include GST?</summary>
                <div className="px-4 pb-4"><p className="text-sm text-slate-600">GST is calculated at 15% by default. Toggle the "Include tax" checkbox in document settings to show or hide tax entirely. You can change the rate and name when enabled. Adjust line item rates to work ex-GST or inc-GST as needed.</p></div>
              </details>
              <details className="rounded-xl border border-slate-200 bg-white">
                ...
                <div className="px-4 pb-4"><p className="text-sm text-slate-600">Yes. The currency selector in the document settings bar supports NZD, AUD, USD, GBP, EUR, and CAD. All amounts in the form and the generated invoice will use the selected currency symbol.</p></div>
              </details>
              <details className="rounded-xl border border-slate-200 bg-white">
              ...
              <details className="rounded-xl border border-slate-200 bg-white">
                ...
                <div className="px-4 pb-4"><p className="text-sm text-slate-600">Sign up at the top of the page to gain more free invoices and remove the QuoteCore+ branding, or sign up to the full QuoteCore+ app for higher limits and loads of extra features.</p></div>
              </details>
            </div>
          </div>
        </section>
